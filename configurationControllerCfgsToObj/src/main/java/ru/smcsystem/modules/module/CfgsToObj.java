...
public class CfgsToObj implements Module {
                    ...
                    .forEach(messagesList -> {
                        try {
                            ...
                            switch (ModuleUtils.getNumber(messages.poll()).intValue()) {
                                case 1:
                                    processRead(executionContextTool, size);
                                    break;
                                case 2:
                                    processSet(configurationTool, executionContextTool, size, messages);
                                    break;
                            }
                        } catch (Exception e) {
    ...
    private void processSet(ConfigurationTool configurationTool, ExecutionContextTool executionContextTool, int size, LinkedList<IMessage> messages) {
        ObjectArray objectArray = ModuleUtils.deserializeToObject(messages);
        if (!ModuleUtils.isArrayContainObjectElements(objectArray))
            return;
        ((ObjectElement) objectArray.get(0)).getFields().stream()
                .filter(f -> f.getValue() != null)
                .forEach(f -> paramList.stream()
                                            ...
                                            if (ModuleUtils.isObjectElement(f)) {
                                                ...
                                                switch (ModuleUtils.toValueType(f)) {
                                                    ...
                                                    case BYTES:
                                                        ...
                                                        break;
                                                    case BOOLEAN:
                                                        iConfigurationManaged.setSetting(p.getName(), (Boolean) f.getValue());
                                                        break;
                                                }
                                            }
                                        }
    ...
    private void processRead(ExecutionContextTool executionContextTool, int size) {
